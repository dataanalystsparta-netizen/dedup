...
with st.sidebar:
    ...
    # CHANGED DEFAULT TO 127.0.0.1 TO RESOLVE ERRNO 99
    db_host = st.text_input("DB Host:", value="127.0.0.1", help="Use 127.0.0.1 instead of localhost to bypass local IPv6 socket mapping issues.")
...
def test_db_connection(host, port, user, password, dbname):
    """Attempts a clean diagnostic query against the target database."""
    ...
    try:
        ...
        # Execute a lightweight diagnostic raw SQL text check
        with engine.connect() as conn:
            conn.execute(text("SELECT 1"))
        return True, "Connection successful! Database is responding normally."
    except Exception as e:
        return False, str(e)

# Direct database verification UI component
st.markdown("### 🔍 Step 1: Database Status Verification")
db_verified = False

if st.button("⚡ Test DB Connection"):
    with st.spinner("Pinging database..."):
        is_valid, msg = test_db_connection(db_host, db_port, db_user, db_pass, db_name)
        if is_valid:
            st.success(msg)
...
# Retain connection state between interface interactions
if st.session_state.get("db_verified", False):
    ...
    def robust_load_file(uploaded_file, password=None):
        """Attempts to safely read an uploaded file across formats."""
    ...
    if uploaded_file is not None:
        try:
            ...
            if st.button("🚀 Run Database Suppression"):
                with st.spinner("Filtering new unique phone numbers..."):
