...
def robust_load_file(uploaded_file, password=None):
    """Safely reads uploaded files matching multiple extensions and formats."""
...
if uploaded_file is not None:
    try:
        ...
        # Auto-detect if a column contains 'phone' or 'tel' to save time for non-tech users
        default_idx = 0
        ...
        if st.button("🚀 Run Database Suppression"):
            with st.spinner("Connecting to MySQL and pulling existing records..."):
                ...
                # Securely stream just the targeted phone numbers from the DB
                query = f"SELECT `{db_phone_col}` FROM `{db_table}`"
                db_phones_df = pd.read_sql(query, con=engine)
                
                # Format database numbers uniformly to match the uploaded data
